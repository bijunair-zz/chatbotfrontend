...
@bp.route("/chat_org", methods=["POST"])
async def chat():
    if not request.is_json:
        return jsonify({"error": "request must be json"}), 415
    request_json = await request.get_json()
...
def make_api_call():
    url = 'https://nmgchatbot.eastus.inference.ml.azure.com/score'
    headers = {
        'Authorization': 'Bearer L1q0dDHGHymqgah1na6dcLmZ6604bF7z',
        'Content-Type': 'application/json',
        'azureml-model-deployment': 'nmgchatbot'
    }
    print(f"Making API call to{url} with request body:{request_body}")
